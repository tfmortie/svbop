...
{
	dst = new T[static_cast<unsigned long>(n)];
	memcpy((void *)dst,(void *)src,sizeof(T)*static_cast<unsigned long>(n));
}
#define INF HUGE_VAL
...
l2r_lr_fun::l2r_lr_fun(const problem *prob, double *C)
{
	...
	z = new double[static_cast<unsigned long>(l)];
	D = new double[static_cast<unsigned long>(l)];
	this->C = C;
}
...
l2r_l2_svc_fun::l2r_l2_svc_fun(const problem *prob, double *C)
{
	...
	z = new double[static_cast<unsigned long>(l)];
	I = new int[static_cast<unsigned long>(l)];
	this->C = C;
}
...
Solver_MCSVM_CS::Solver_MCSVM_CS(const problem *prob, int nr_class, double *weighted_C, double eps, int max_iter)
{
	...
	this->prob = prob;
	this->B = new double[static_cast<unsigned long>(nr_class)];
	this->G = new double[static_cast<unsigned long>(nr_class)];
	this->C = weighted_C;
}
...
void Solver_MCSVM_CS::solve_sub_problem(double A_i, int yi, double C_yi, int active_i, double *alpha_new)
{
	...
	if(yi < active_i)
		D[yi] += A_i*C_yi;
	qsort(D, static_cast<unsigned long>(active_i), sizeof(double), compare_double);
...
void Solver_MCSVM_CS::Solve(double *w)
{
	int i, m, s;
	int iter = 0;
	double *alpha =  new double[static_cast<unsigned long>(l)*static_cast<unsigned long>(nr_class)];
	double *alpha_new = new double[static_cast<unsigned long>(nr_class)];
	int *index = new int[static_cast<unsigned long>(l)];
	double *QD = new double[static_cast<unsigned long>(l)];
	int *d_ind = new int[static_cast<unsigned long>(nr_class)];
	double *d_val = new double[static_cast<unsigned long>(nr_class)];
	int *alpha_index = new int[static_cast<unsigned long>(nr_class)*static_cast<unsigned long>(l)];
	int *y_index = new int[static_cast<unsigned long>(l)];
	int active_size = l;
	int *active_size_i = new int[static_cast<unsigned long>(l)];
	...
	double Cp, double Cn, int solver_type)
{
	...
	int max_iter = 1000;
	int *index = new int[static_cast<unsigned long>(l)];
	double *alpha = new double[static_cast<unsigned long>(l)];
	schar *y = new schar[static_cast<unsigned long>(l)];
	int active_size = l;
	...
	int solver_type)
{
	...
	double *beta = new double[static_cast<unsigned long>(l)];
	double *QD = new double[static_cast<unsigned long>(l)];
	double *y = prob->y;
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	int w_size = prob->n;
	int i, s, iter = 0;
	double *xTx = new double[static_cast<unsigned long>(l)];
	int max_iter = 1000;
	int *index = new int[static_cast<unsigned long>(l)];
	double *alpha = new double[2*static_cast<unsigned long>(l)]; // store alpha and C - alpha
	schar *y = new schar[static_cast<unsigned long>(l)];
	...
	double Cp, double Cn)
{
	...
	int *index = new int[static_cast<unsigned long>(w_size)];
	schar *y = new schar[static_cast<unsigned long>(l)];
	double *b = new double[static_cast<unsigned long>(l)]; // b = 1-ywTx
	double *xj_sq = new double[static_cast<unsigned long>(w_size)];
	feature_node *x;
	...
	double Cp, double Cn)
{
	...
	int *index = new int[static_cast<unsigned long>(w_size)];
	schar *y = new schar[static_cast<unsigned long>(l)];
	double *Hdiag = new double[static_cast<unsigned long>(w_size)];
	double *Grad = new double[static_cast<unsigned long>(w_size)];
	double *wpd = new double[static_cast<unsigned long>(w_size)];
	double *xjneg_sum = new double[static_cast<unsigned long>(w_size)];
	double *xTd = new double[static_cast<unsigned long>(l)];
	double *exp_wTx = new double[static_cast<unsigned long>(l)];
	double *exp_wTx_new = new double[static_cast<unsigned long>(l)];
	double *tau = new double[static_cast<unsigned long>(l)];
	double *D = new double[static_cast<unsigned long>(l)];
	feature_node *x;
...
static void transpose(const problem *prob, feature_node **x_space_ret, problem *prob_col)
{
	...
	int n = prob->n;
	size_t nnz = 0;
	size_t *col_ptr = new size_t [static_cast<unsigned long>(n)+1];
	feature_node *x_space;
	prob_col->l = l;
	prob_col->n = n;
	prob_col->y = new double[static_cast<unsigned long>(l)];
	prob_col->x = new feature_node*[static_cast<unsigned long>(n)];
	...
	x_space = new feature_node[nnz+static_cast<unsigned long>(n)];
	for(i=0; i<n; i++)
...
static void group_classes(const problem *prob, int *nr_class_ret, int **label_ret, int **start_ret, int **count_ret, int *perm)
{
	...
	int max_nr_class = 16;
	int nr_class = 0;
	int *label = Malloc(int,static_cast<unsigned long>(max_nr_class));
	int *count = Malloc(int,static_cast<unsigned long>(max_nr_class));
	int *data_label = Malloc(int,static_cast<unsigned long>(l));
	int i;
	...
	for(i=0;i<l;i++)
	{
		...
		if(j == nr_class)
		{
			...
			{
				max_nr_class *= 2;
				label = (int *)realloc(label,static_cast<unsigned long>(max_nr_class)*sizeof(int));
				count = (int *)realloc(count,static_cast<unsigned long>(max_nr_class)*sizeof(int));
			}
	...
	int *start = Malloc(int,static_cast<unsigned long>(nr_class));
	start[0] = 0;
...
static void train_one(const problem *prob, const parameter *param, double *w, double Cp, double Cn)
{
	...
	switch(param->solver_type)
	{
		case L2R_LR:
		{
			double *C = new double[static_cast<unsigned long>(prob->l)];
			for(int i = 0; i < prob->l; i++)
			{
				if(prob->y[i] > 0)
					C[i] = Cp;
...
static void find_parameter_C(const problem *prob, parameter *param_tmp, double start_C, double max_C, double *best_C, double *best_score, const int *fold_start, const int *perm, const problem *subprob, int nr_fold)
{
	// variables for CV
	int i;
	double *target = Malloc(double, static_cast<unsigned long>(prob->l));

	// variables for warm start
	double ratio = 2;
	double **prev_w = Malloc(double*, static_cast<unsigned long>(nr_fold));
	for(i = 0; i < nr_fold; i++)
		prev_w[i] = NULL;
	...
	*best_C = start_C;
	...
	while(param_tmp->C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			if(prev_w[i] == NULL)
			{
				prev_w[i] = Malloc(double, static_cast<unsigned long>(total_w_size));
...
model* train(const problem *prob, const parameter *param)
{
	...
	{
		model_->w = Malloc(double, static_cast<unsigned long>(w_size));
	...
	else
	{
		...
		int *start = NULL;
		int *count = NULL;
		int *perm = Malloc(int,static_cast<unsigned long>(l));

		// group training data of the same class
		group_classes(prob,&nr_class,&label,&start,&count,perm);

		model_->nr_class=nr_class;
		model_->label = Malloc(int,static_cast<unsigned long>(nr_class));
		for(i=0;i<nr_class;i++)
			model_->label[i] = label[i];

		// calculate weighted C
		double *weighted_C = Malloc(double, static_cast<unsigned long>(nr_class));
		for(i=0;i<nr_class;i++)
		...
		feature_node **x = Malloc(feature_node *,static_cast<unsigned long>(l));
		for(i=0;i<l;i++)
			x[i] = prob->x[perm[i]];
		...
		sub_prob.l = l;
		sub_prob.n = n;
		sub_prob.x = Malloc(feature_node *,static_cast<unsigned long>(sub_prob.l));
		sub_prob.y = Malloc(double,static_cast<unsigned long>(sub_prob.l));
		...
		{
			model_->w=Malloc(double, static_cast<unsigned long>(n)*static_cast<unsigned long>(nr_class));
			for(i=0;i<nr_class;i++)
		...
		else
		{
			...
			{
				model_->w=Malloc(double, static_cast<unsigned long>(w_size)*static_cast<unsigned long>(nr_class));
				double *w=Malloc(double, static_cast<unsigned long>(w_size));
				for(i=0;i<nr_class;i++)
				{
					int si = start[i];
					int ei = si+count[i];
...
void cross_validation(const problem *prob, const parameter *param, int nr_fold, double *target)
{
	...
	fold_start = Malloc(int,static_cast<unsigned long>(nr_fold)+1);
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		subprob.x = Malloc(struct feature_node*,static_cast<unsigned long>(subprob.l));
		subprob.y = Malloc(double,static_cast<unsigned long>(subprob.l));
...
void find_parameters(const problem *prob, const parameter *param, int nr_fold, double start_C, double start_p, double *best_C, double *best_p, double *best_score)
{
	...
	int l = prob->l;
	int *perm = Malloc(int, static_cast<unsigned long>(l));
	struct problem *subprob = Malloc(problem,static_cast<unsigned long>(nr_fold));
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		subprob[i].x = Malloc(struct feature_node*,static_cast<unsigned long>(subprob[i].l));
		subprob[i].y = Malloc(double,static_cast<unsigned long>(subprob[i].l));
...
{
	double *dec_values = Malloc(double, static_cast<unsigned long>(model_->nr_class));
...
struct model *load_model(const char *model_file_name)
{
	...
	model_->w=Malloc(double, static_cast<unsigned long>(w_size)*static_cast<unsigned long>(nr_w));
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
