...
void TRON::tron(double *w)
{
	...
	double *s = new double[static_cast<unsigned long>(n)];
	double *r = new double[static_cast<unsigned long>(n)];
	double *g = new double[static_cast<unsigned long>(n)];

	const double alpha_pcg = 0.01;
	double *M = new double[static_cast<unsigned long>(n)];

	// calculate gradient norm at w=0 for stopping condition.
	double *w0 = new double[static_cast<unsigned long>(n)];
	for (i=0; i<n; i++)
		w0[i] = 0;
	fun_obj->fun(w0);
	fun_obj->grad(w0, g);
	...
	double *w_new = new double[static_cast<unsigned long>(n)];
	bool reach_boundary;
	...
	while (iter <= max_iter && search)
	{
		...
		memcpy(w_new, w, sizeof(double)*static_cast<unsigned long>(n));
		...
		{
			iter++;
			memcpy(w, w_new, sizeof(double)*static_cast<unsigned long>(n));
			f = fnew;
			fun_obj->grad(w, g);
			fun_obj->get_diag_preconditioner(M);
			for(i=0; i<n; i++)
...
int TRON::trpcg(double delta, double *g, double *M, double *s, double *r, bool *reach_boundary)
{
	...
	int n = fun_obj->get_nr_variable();
	double one = 1;
	double *d = new double[static_cast<unsigned long>(n)];
	double *Hd = new double[static_cast<unsigned long>(n)];
	double zTr, znewTrnew, alpha, beta, cgtol;
	double *z = new double[static_cast<unsigned long>(n)];
